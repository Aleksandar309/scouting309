...
const formSchema = z.object({
  ...
  type: z.enum(["Freewatch", "Player Tracking", "Area Tracking"], { message: "Please select a task type." }),
...
const TaskForm: React.FC<TaskFormProps> = ({ onAddTask, onClose, scouts }) => {
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    onAddTask(newTask);
    toast.success(`Task "${newTask.title}" created for ${newTask.assignedToName}!`);
    onClose();
  };
    ...
    <DialogContent className="sm:max-w-[600px] bg-card text-card-foreground border-border max-h-[90vh] overflow-y-auto">
      <DialogHeader>
        ...
        <DialogDescription className="text-muted-foreground">
          Assign a new scouting task to one of your scouts.
        </DialogDescription>
      </DialogHeader>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="grid gap-4 py-4">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="text-muted-foreground">Task Title</FormLabel>
                <FormControl>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="text-muted-foreground">Task Type</FormLabel>
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  <FormControl>
                    <SelectTrigger className="bg-input border-border text-foreground">
                  ...
                  <SelectContent className="bg-popover border-border text-popover-foreground">
                    <SelectItem value="Freewatch">Freewatch</SelectItem>
                    <SelectItem value="Player Tracking">Player Tracking</SelectItem>
                    <SelectItem value="Area Tracking">Area Tracking</SelectItem>
                  </SelectContent>
                </Select>
                <FormMessage />
              </FormItem>
                  ...
                  <Popover>
                    ...
                    <PopoverContent className="w-auto p-0 bg-popover border-border text-popover-foreground" align="start">
                      <Calendar
                        ...
                        disabled={(date) => date < new Date("1900-01-01")} // Allow past dates for historical tasks
                        initialFocus
                      />
                    </PopoverContent>
                  </Popover>
