...
const PlayerPitch: React.FC<PlayerPitchProps> = ({ positionsData, formationPositions, onPositionClick }) => {
  ...
  if (!positionsToRender) {
    return (
      <div className="relative w-full aspect-[3/2] max-w-[600px] mx-auto bg-background border-2 border-border rounded-lg overflow-hidden shadow-inner flex items-center justify-center text-muted-foreground">
  ...
  return (
    <TooltipProvider>
      ...
      <div className="relative w-full aspect-[3/2] max-w-[600px] mx-auto bg-background border-2 border-border rounded-lg overflow-hidden shadow-inner">
        {/* Pitch Lines */}
        <div className="absolute inset-0">
          {/* Outer border (already on parent div) */}

          {/* Halfway line (vertical) */}
          <div className="absolute top-0 bottom-0 left-1/2 w-0.5 bg-pitch-line"></div>
          {/* Center circle */}
